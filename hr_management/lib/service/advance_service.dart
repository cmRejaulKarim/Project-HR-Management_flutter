...
class AdvanceService {
  ...
  Future<AdvanceSalary?> addAdvanceRequest(double amount, String reason) async {
    ...
    if (response.statusCode == 200) {
      ...
      print(
        'Failed to add advance request: ${response.statusCode} ${response.body}',
      );
      return null;
    }
  }
  ...
  Future<List<AdvanceSalary>> viewAdvanceRequestsByEmp() async {
    ...
    if (response.statusCode == 200) {
      ...
      print(
        'Failed to fetch monthly advance requests by emp: ${response.statusCode}',
      );
      return [];
    }
  }
  ...
  Future<List<AdvanceSalary>> viewYearlyAdvanceRequestsByEmp() async {
    ...
    if (response.statusCode == 200) {
      ...
      print(
        'Failed to fetch yearly advance requests by emp: ${response.statusCode}',
      );
      return [];
    }
  }
  ...
  Future<List<AdvanceSalary>> viewAdvanceRequests(int empId) async {
    ...
    if (response.statusCode == 200) {
      ...
      print(
        'Failed to fetch advance requests for employee $empId: ${response.statusCode}',
      );
      return [];
    }
  }
  ...
  Future<AdvanceSalary?> getApprovedAdvanceForMonth(
    int empId,
    String date,
  ) async {
    final headers = await _getHeaders();
    final uri = Uri.parse(
      '$baseUrl/employee/$empId/approved',
  ...
  Future<List<AdvanceSalary>> getAdvancesInPeriod(
    int empId,
    String startDate,
    String endDate,
  ) async {
